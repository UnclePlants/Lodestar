...
- **Initiative tracker** — a dockable turn-order panel: add players, NPCs, and monsters with initiative and HP, advance turns and rounds, track damage/healing with HP bars. A compact order overlay can be shown in the corner of the GM screen and/or the player display independently; it stays put when the panel is closed and has its own turn arrows so you can advance the round without reopening the panel; each row also has a **K** toggle to mark a combatant as concentrating, mirrored as a badge in the order overlay for GM and players alike.
